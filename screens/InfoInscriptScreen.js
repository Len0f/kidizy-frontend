...
export default function InfoInscriptScreen({ navigation }) {
  ...
  const [th, setTH] = useState(""); // taux horaire (babysitter)
  const [lastEnfant, setLastEnfant] = useState([{ firstName: "", age: "" }]);

  // --------- États pour uploads / aperçus
  ...
  // --------- Texte libre (babysitter)
  const [Biographie, setBiographie] = useState("");
  const [Interest, setInterest] = useState("");

  // --------- Autocomplétion adresse (API Adresse data.gouv)
  const [addressSuggestions, setAddressSuggestions] = useState([]);
  const [showSuggestions, setShowSuggestions] = useState(false);
  const debounceRef = useRef(null);

  // Appel à l’API publique pour suggérer des adresses (limite 5)
  ...
  // Debounce de la recherche (300ms après dernière frappe)
  ...
  // Saisie adresse : on relance l’autocomplétion + reset lat/lon
  ...
  // Sélection d’une suggestion -> verrouille adresse + remplit lat/lon
  ...
  // Rendu d’un item de suggestion d’adresse
  ...
  // --------- Sélection + upload d’image (avatar / CNI / Casier)
  const pickAndUpload = async (type) => {
    try {
      ...
      // Enregistre l’URL dans l’état approprié
  ...
  // --------- Soumission du formulaire (maj profil)
  const handleSubmit = async () => {
    ...
    // Structure unifiée : certaines sous-parties ne seront utilisées que selon le rôle
    const payload = {
      firstName: prenom,
      lastName: nom,
      phone: telephone,
    ...
    try {
      ...
      if (res.result) {
        navigation.navigate("TabNavigator"); // retour au tableau de bord
      } else {
  ...
  // --------- Gestion dynamique de la liste d’enfants (parent)
  const handleAdd = () =>
  ...
  // Rendu des champs “enfant” (firstName/age) pour chaque entrée
  ...
  // --------- Styles conditionnels selon le profil
  let UserStyle;
  let color;
  if (profil === "parent") {
    color = { color: "#98C2E6" };
    ...
    >
      ...
      >
        <View style={styles.containeImage}>
          ...
          <TouchableOpacity
            style={[
              styles.containePhoto,
              { backgroundColor: profil === "parent" ? "#9FC6E7" : "#88E19D" },
            ]}
        ...
        {profil === "parent" ? (
          ...
          <>
            ...
            <View style={styles.containeInput}>
              <TouchableOpacity
                style={[styles.uploadBtn, { backgroundColor: "#88E19D" }]}
            ...
            <View style={styles.containeInput}>
              ...
              >
                <Text style={styles.uploadText}>
                  {casierUrl ? "Casier importé" : "Importer Casier judiciaire"}
                </Text>
              </TouchableOpacity>
