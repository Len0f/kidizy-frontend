import { Button, SafeAreaView, StyleSheet, Text, Image, View, ScrollView, TouchableOpacity, KeyboardAvoidingView, Platform, Alert, FlatList } from 'react-native';
...
export default function ProfilScreen({ navigation }) {
  ...
  // Fetch profil => récupérer adresse + lat/lon si dispo
  useEffect(() => {
    if (!userToken) return;
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      location: adresse ? { address: adresse, lat: lat || '', lon: lon || '' } : undefined,
    };
  ...
  const addEnfant = lastEnfant.map((_, i) => (
    <View key={i} style={styles.containeInput}>
      ...
      <View style={styles.inputAge}>
        <Input width="100%" placeholder={lastEnfant[i].age} name="Age" setText={(val) => modifAge(val, i)} text={lastEnfant[i].age} />
      </View>
    </View>
  ));
          ...
          <ScrollView keyboardShouldPersistTaps="handled">
            <View style={styles.avatarContainer}>
              ...
              >
                <Image
                  style={styles.photo}
                  source={avatarLocal ? { uri: avatarLocal } : (avatarUrl ? { uri: avatarUrl } : require('../assets/babysitter2.png'))}
                />
              </TouchableOpacity>
            ...
            <View style={styles.mainContent}>
              ...
              <View style={styles.containeInput}>
                <Input style={styles.inputTH} userStyle={{ color: color }} placeholder={String(th)} width="60%" name="Taux horaire" setText={setTH} text={String(th)} />
              </View>
              <View style={styles.containeInput}>
            ...
            <View style={styles.userReview}>
              <View style={styles.userRating}>
                <View className="stars" style={styles.stars}>
                  {[1,2,3,4,5].map(i => (
                    <FontAwesome key={i} name="star" size={18} color={i <= personalStar ? color : '#323232'} />
                  ))}
                </View>
                <Text style={styles.ratingText}>(11)</Text>
              </View>
            ...
            <View style={styles.mainContent}>
              ...
              <View style={styles.containeEnfant}>
                <View style={styles.containeBtnEnfant}>
                  <TouchableOpacity style={styles.btnContainer} onPress={() => setLastEnfant([...lastEnfant, { firstName: '', age: '' }])}>
                ...
                <View style={styles.containeInputEnfant}>
                  {lastEnfant.map((_, i) => (
                    <View key={i} style={styles.containeInput}>
                      <Input style={styles.inputEnfant} width="41%" placeholder={lastEnfant[i].firstName} name="Enfant" setText={(prenom) => setLastEnfant(prev => prev.map((e, t) => t === i ? { firstName: prenom, age: e.age } : e))} text={lastEnfant[i].firstName} />
                      <View style={styles.inputAge}>
                        <Input width="100%" placeholder={lastEnfant[i].age} name="Age" setText={(val) => setLastEnfant(prev => prev.map((e, t) => t === i ? { firstName: e.firstName, age: val } : e))} text={lastEnfant[i].age} />
                      </View>
                    </View>
                  ))}
                </View>
