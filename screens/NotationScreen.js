...
console.log('info garde',infoGarde)
...
useEffect(()=>{
  ...
  const handleSubmit = () => {
   fetch(`${url}gardes/${infoGarde._id}/rating`, {
     ...
     .then(data => {console.log('clic', data);
       if (data.result) {
  ...
  return (
    ...
    <View style={styles.summaryCard}>
      <Text style={[styles.summaryTitle, { color }]}>Résumé de la garde</Text>
      ...
      <View style={styles.summaryRow}>
        ...
        <Text style={styles.summaryValue}>{`${infoGarde.idUserBabysitter.babysitterInfos.price}/h`}</Text>
      </View>

      <View style={[styles.summaryRow, styles.summaryTotalRow]}>
        <Text style={[styles.summaryLabel, styles.summaryTotalLabel]}>{payer}</Text>
        <Text style={[styles.summaryValue, styles.summaryTotalValue]}>{formatCurrency(cost)}</Text>
      </View>
    </View>
    ...
    <View style={styles.containeIB}>
      <View style={styles.containeInput}>
        <Input name="Avis" setText={setText} text={text} userStyle={{ color }} />
      </View>
      <View style={styles.containeBtn}>
        <MainBtn userStyle={{ backgroundColor: color }} btnTitle="Soumettre" clickNav={handleSubmit} />
      </View>
    </View>
  </ScrollView>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#FFFBF0', alignItems: 'center', justifyContent: 'center' },
  avatarContainer:{ justifyContent:'center', alignItems:'center', marginTop: 10, marginBottom: 10 },
  avatar:{ borderRadius: 100, width:150, height:150, objectFit:'contain', margin:10 },
  avatarName:{ fontFamily:'Montserrat', fontSize:30, fontWeight:'700' },

  // ---- Résumé
  summaryCard: {
    width: '90%', alignSelf: 'center', backgroundColor: '#fff', borderRadius: 12,
    paddingVertical: 14, paddingHorizontal: 16, borderWidth: 1, borderColor: '#e6e2d9',
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4,
    elevation: 2, marginBottom: 18,
  },
  summaryTitle: { fontFamily:'Montserrat', fontSize: 18, fontWeight: '700', marginBottom: 8 },
  summaryRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6 },
  summaryLabel: { fontFamily:'Montserrat', fontSize: 14, color: '#444' },
  summaryValue: { fontFamily:'Montserrat', fontSize: 14, color: '#111', fontWeight: '700' },
  summaryTotalRow: { borderTopWidth: 1, borderTopColor: '#eee', marginTop: 6, paddingTop: 10 },
  ...
  etoiles: { flexDirection: "row", justifyContent:'center', alignItems:'center', marginBottom: 25 },
  containeIB: { flexDirection: "column", justifyContent:'center', alignItems:'center' },
