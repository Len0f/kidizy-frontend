...
export default function PropositionScreen({ navigation, route }) {
    ...
    const [babysitterId, setBabysitterId] = useState(babysitterFromParams|| user.selectedBabysitterId || '');
...
// ----------------- PARENT : préremplir depuis route.params
    useEffect(() => {
        ...
        setBabysitterId((prev) => prev || babysitterFromParams || user.selectedBabysitterId || '');
      }, [isParent, firstNameParam, lastNameParam, dayParam, hoursParam, kidsParam, commentParam, babysitterFromParams, user.id, user.selectedBabysitterId]);
    ...
    useEffect(() => {
        if (isBabysitter && proposition) {
          ...
          .then(data => {
              ...
              setParentId(data.propo.idUserParent?._id || data.propo.idUserParent || parentId);
    ...
    const createProposition = async() => {
        try {
            ...
            if (res?.result) {
                ...
                Alert.alert('Erreur', res?.error || 'Impossible de créer la proposition.');
            }
        } catch (e) {
...
// ------------------ UTILITAIRE : CREATION DE LA CONVERSATION
    ...
    if(resConv?.result) {
        navigation.navigate('Chat', { conversation: resConv.conversationId._id, from: 'Contacts', profil });
    } else {
        returnScreen();
    }
};
            ...
            {isParent ? (
                ...
                <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
                    ...
                    <View style={styles.mapContainer}>
                        ...
                        >
                            <Marker 
                                coordinate={{ latitude: 44.8643352091005, longitude: -0.5760245233606299 }}
                                title={prenom}
                            />
                        </MapView>
                    </View>
