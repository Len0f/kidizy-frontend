...
navigation.navigate('Notation', {
  ...
  // --------- Données affichées (mock)
//   const data = { Prenom: 'Josiane', Nom: 'Pichet', Jour: '09/08', Horaires: '19H00-23H00', Enfant: 'Gregory' };
//   const keys = Object.keys(data);
  ...
  const resolvedLocation = useMemo(() => {
    ...
    if (lat !== null && lon !== null) return { latitude: lat, longitude: lon, address: src?.address || '' };
    ...
    >
      {hasValidCoords && (
        <Marker
          ...
          title={resolvedLocation.address || infoGarde.proposition.firstName
}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#FFFBF0', position: 'relative', alignItems: 'center' },
  btnReturnContainer: { position: 'absolute', zIndex: 10, left: 0, marginLeft: 20 },
  screenTitleContainer: { marginVertical: 50, justifyContent: 'center' },
  screenTitle: { fontFamily: 'Montserrat', fontSize: 28, fontWeight: '700' },
  buttons: { alignItems: 'flex-end', width: '90%', marginVertical: 10, justifyContent: 'space-between' },
  btn: { marginBottom: 20, borderWidth: 1 },
  mainContent: { marginTop: 10, flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center' },
  map: { flex: 1, borderRadius: 8 },
  mapContainer: { height: 130, width: '90%', marginVertical: 10 },
  recenterBtn: { position: 'absolute', right: 10, bottom: 10, backgroundColor: 'white', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, borderWidth: 1, borderColor: '#e3e3e3' },
