...
// Ce composant gère toute la barre de filtres pour la recherche de babysitters.
// Il propose plusieurs critères : note, distance, âge, disponibilité (jour / heures) et tri.
// Chaque filtre ouvre un Modal listant les options disponibles.
// Les choix sélectionnés sont envoyés au parent via des props setters (ex: setNoteFilter).

export default function FilterBar(props) {
  const [modalVisible, setModalVisible] = useState(false); // Ouverture/fermeture du modal
  const [currentFilter, setCurrentFilter] = useState(null); // Type de filtre selectionné.

  // Options de filtres proposées à l’utilisateur
  const filterData = {
    note: [
      "Toutes",
      "0 étoile",
  ...
  // Ouvre le modal pour le type de filtre demandé
  const openModal = (filterType) => {
    setCurrentFilter(filterType);
    setModalVisible(true);
  };

  // Lorsqu’on choisit une option dans le modal
  const selectOption = (value) => {
    console.log("FilterBar selection:", currentFilter, value);

    if (currentFilter === "note") {
      props.setNoteFilter(
        value === "Toutes" ? "" : Number(value.split(" ")[0])
      ); // Ex: 4 étoiles = 4 (nombre)
    }
  ...
  // Fonction utilitaire : affiche un libellé en fonction du filtre actif
  const getLabel = (type) => {
    if (type === "note") {
  ...
  return (
    <>
      ...
      <View style={styles.row}>
        <TouchableOpacity
          style={styles.filterBtn}
          onPress={() => openModal("availabilityDay")}
        >
        ...
        <TouchableOpacity
          style={styles.filterBtn}
          onPress={() => openModal("availabilityHours")}
        >
